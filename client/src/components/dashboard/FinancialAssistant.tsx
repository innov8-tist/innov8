...
const predictLoanSanction = (data: FormData): boolean => {
  if (data.Credit_History === 0) return false;
  
  const loanIncomeRatio = data.LoanAmount / data.ApplicantIncome;
  if (loanIncomeRatio > 0.5) return false;
  
  if (data.Education === "Not Graduate" && data.Self_Employed === "Yes" && data.ApplicantIncome < 3000) {
    return false;
  }
...
const calculateCreditScore = (data: CreditScoreData): number => {
  ...
  // Debt-to-income ratio factor (lower ratio = higher score)
  ...
  // Loan amount factor (lower loan amount relative to income = higher score)
...
const FinancialAssistant = () => {
  ...
  const isLoanApproved = predictLoanSanction(formData);
  ...
  const creditScore = calculateCreditScore(creditScoreData);
